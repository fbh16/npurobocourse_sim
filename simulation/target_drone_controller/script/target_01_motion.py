...
if __name__ == '__main__':
    ...
    # settings = termios.tcgetattr(sys.stdin)
    rospy.init_node('target_motion_01')
    cmd_vel_pub = rospy.Publisher('/cmd_vel', Twist, queue_size=1)
    # takeoff_pub = rospy.Publisher('/takeoff', Empty, queue_size=1)
    # land_pub = rospy.Publisher("/land", Empty, queue_size=1)
    # drone_id = rospy.get_param('~drone_name', 'target_01')
    
    state = DroneState()    

    altitude = 5.0    
    ax = [0.500, 1.371, 2.243, 3.114, 3.986, 4.857, 5.729, 6.600]
    ay = [-10.000, -7.923, -5.846, -3.769, -1.691, 0.386, 2.463, 4.540]
    ...
    rospy.Subscriber(f'/target_01/ground_truth_to_tf/pose', PoseStamped, callback, queue_size=10)
