...
class PublishThread(threading.Thread):
    def __init__(self, rate):
        ...
        self.publisher = rospy.Publisher('/obs_03/cmd_vel', Twist, queue_size=1)
        self.x = 0.0
        self.y = 0.0
        self.z = 0.0
        self.th = 0.0
...
class DroneController:
    def __init__(self):
        # 目标点
        # self.point_1 = [-3.6, 1.7]
        # self.point_2 = [-3.0, 1.6]
        self.point_1 = [-4.0, 0]
        self.point_2 = [-3.5, 0]
        ...
        # ROS 话题订阅与发布
        self.cmd_vel_pub = rospy.Publisher('/obs_03/cmd_vel', Twist, queue_size=1)
        self.pose_sub = rospy.Subscriber('/vrpn_client_node/obs_03/pose', PoseStamped, self.callback, queue_size=10)
    ...
    def callback(self, msg):
        ...
        ## rise
        if self.current_position[2] < 1.2:
            vel_msg = Twist()
            vel_msg.linear.x = 0
            vel_msg.linear.y = 0
...
if __name__ == '__main__':
    ...
    rospy.init_node('drone_motion_3')

    key_timeout = 0.0
    pub_thread = PublishThread(0.0)
    
    
    takeoff_pub = rospy.Publisher('/obs_03/takeoff', Empty, queue_size=1)
    land_pub = rospy.Publisher("/obs_03/land", Empty, queue_size=1)
    ...
    # rospy.Subscriber('/vrpn_client_node/obs_03/pose', PoseStamped, callback, queue_size=10)
